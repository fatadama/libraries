/* Wii Remote IR Sensor Test Sketch with Bitlash and Logging
   ...
   with external video using audible chirps. You need to already have Bitlash installed in your Arduino libraries directory to compile this sketch. Requires PixArt sensor on I2C bus, SD card adapter on SPI,
   a mid-to-high impedance speaker, and an LED. If you're using this sketch on an Uno or compatible, do
   NOT set the LED pin to the built-in because that pin is shared with the SPI CS line.
...
const int ledPin = 5; // arbitrary LED pin
